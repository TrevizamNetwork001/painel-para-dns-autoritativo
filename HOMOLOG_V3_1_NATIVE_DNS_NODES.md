...
```sh
ssh -F /dev/null   -i /var/www/.ssh/id_ed25519_dns_sync   -o BatchMode=yes   -o IdentitiesOnly=yes   -o StrictHostKeyChecking=yes   -o UserKnownHostsFile=/var/www/.ssh/known_hosts   -o ConnectTimeout=6   -p 22 dns-sync@198.51.100.243   'sudo -n /usr/local/bin/dns-slave-status.sh'
```
...
### Funcao PHP `dns_server_executar_teste(..., transfer, legacy.example)`

Resultado: OK.

```text
zone=legacy.example
soa=ns1.legacy.example. hostmaster.legacy.example. 2026051906 900 3600 2419200 300
authoritative=yes
serial=2026051906
```
