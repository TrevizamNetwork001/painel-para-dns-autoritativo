...
class AgentTests(unittest.TestCase):
    ...
    def test_parse_named_conf_zones_normalizes_master_and_slave(self) -> None:
        ...
        self.assertEqual("conectanetwork.net.br", zones[0]["name"])
        self.assertEqual("primary", zones[0]["detected_type"])
        self.assertEqual("master", zones[0]["detected_syntax"])
        self.assertEqual(
            "/var/cache/bind/master-aut/conectanetwork.net.br.hosts",
            zones[0]["file"],
        )
    ...
    def test_parse_canonical_zone_dump_handles_soa_owner_continuation_and_txt(
        ...
        dump = (
            ...
            "www\t\t\t3600\tIN\tA\t45.162.196.242\n"
            "\t\t\t3600\tIN\tA\t45.162.196.243\n"
            "mail\t\t\t3600\tIN\tMX\t10 mail.conectanetwork.net.br.\n"
            'txt1\t\t\t3600\tIN\tTXT\t"v=spf1 -all"\n'
        )

        parsed = agent.parse_canonical_zone_dump(dump, "conectanetwork.net.br")
        ...
        self.assertEqual("10 mail.conectanetwork.net.br.", mx_record["rdata"])
    ...
    def test_parse_canonical_zone_dump_handles_ptr_and_reverse_zone(self) -> None:
        dump = (
            "$ORIGIN 196.162.45.in-addr.arpa.\n"
            "$TTL 3600\n"
            "1\t3600\tIN\tPTR\tns1.conectanetwork.net.br.\n"
            "2\t3600\tIN\tPTR\tmail.conectanetwork.net.br.\n"
        )

        parsed = agent.parse_canonical_zone_dump(dump, "196.162.45.in-addr.arpa")
    ...
    def test_discover_bind_zones_refuses_truncated_checkzone_dump_instead_of_parsing_partial_content(
        ...
        # Regression for the real conectanetwork.net.br case: a 1030-record
